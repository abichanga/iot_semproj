...
| BH1750 Light Intensity Sensor | 1–65535 lux, I²C digital output, 16-bit | Sunlight exposure (integrated over hours) |

### 2.3 Display

| Component | Specification | Role |
|---|---|---|
| 1.3" White IIC 128×64 OLED LCD (SH1106) | I²C interface (SDA, SCL), 3.3 V–5 V compatible | Local display for real-time sensor readings at the greenhouse |
...
| 10 kΩ + 20 kΩ resistors (voltage divider) | Divides the MQ-5 analog output (~5 V max) down to ≤ 3.3 V for the ESP32's ADC input. Ratio: 20 k / (10 k + 20 k) ≈ 0.67 |
...
|---|---|---|---|
| 1 | **1.3" White IIC 128×64 OLED LCD** | [1.3inch SH1106 OLED Datasheet (PDF)](https://www.waveshare.com/w/upload/e/e3/1.3inch-SH1106-OLED.pdf) | Waveshare — covers the SH1106 driver IC, I²C/SPI interface configuration, pin definitions, and mechanical drawings |
...
In this design, a single ESP32-S board handles all sensing and display duties. The MQ-5 connects to an ADC-capable GPIO through the voltage divider, the DHT22 connects to a digital GPIO with a pull-up, and the 1.3" OLED connects over the I²C bus (GPIO 21 for SDA, GPIO 22 for SCL).
...
| GPIO 21 (SDA) | OLED SDA | I²C data line |
| GPIO 22 (SCL) | OLED SCL | I²C clock line |
...
| ESP32-A | GPIO 26 | Relay IN (active LOW) | Pulling LOW energises the relay coil |
...
**Relay Logic:**
- `GPIO 26 = HIGH` (default) → relay coil de-energised → NO contact open → ESP32-B is unpowered
- `GPIO 26 = LOW` (triggered) → relay coil energised → NO contact closes → ESP32-B boots and begins MQ-5 readings
